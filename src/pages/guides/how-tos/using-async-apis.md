...
Start with this example that uses the asynchronous version of the text to image endpoint, see [Generate Image Async API Reference](../../api/index.md). For now we won't show authentication which is the same as it is for synchronous calls.
...
The asynchronous API are even more powerful; in this next example, we take a source image, upload it, and then use the [Expand Image Asynchronous API](../../api/index.md) to resize it. Instead of doing one resize after another, we can kick off multiple jobs at once so we can resize an image much more efficiently.
